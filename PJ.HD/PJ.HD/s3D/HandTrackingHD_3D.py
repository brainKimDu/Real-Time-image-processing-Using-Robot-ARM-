...
def HandTrackingHD(RoboAngle,Azimuth,Ldata,CamNum, CmPort):
    grab = False
    stop = False
    Az = 0
    ...
    while True:
        ...
        if(x==0 and y==480):
            ...
            if( (XXPYY <= WorkspaceMaxRadiusSQR) and (XXPYY >= WorkspaceMinRadiusSQR) ):
                ...
                if(grab==True):
                    ...
                    G=',0'
                #2차원 움직임의 경우 Trans = str(Azimuth[x][0]) + ',' + str(RoboAngle[x][y])+G
                if not(stop):
                    Trans = 'f'+ str(Az) + ',' + str(RoboAngle[x][y])+G
                #Cm.send(ARD,Trans)
                print('각데이터전송:'+ Trans)
            else:
        ...
        if (args.display > 0):
            ...
            if k == 27:
                ...
                print("G:"+str(grab))
            elif k==78 or k==110 and Az>10:
                Az -= 5
                print("Azimuth:"+ str(Az))
            elif k==77 or k==109 and Az<360:
                Az += 5
                print("Azimuth:"+ str(Az))
            elif k == 44:
                stop=not(stop and True)
            else:
                print(k)
        else:
